...
async function mapPlace(row) {
  ...
  const stats = statsMap.get(row.id) || { tiolaCount: 0, tiolaRating: null };
  return mapPlaceRow(row, stats);
}

async function filterAndSort(req, { limit, offset } = {}) {
  const {
  ...
  const statsMap = await getStatsMap();
  const mapOne = (row) => mapPlaceRow(row, statsMap.get(row.id) || { tiolaCount: 0, tiolaRating: null });
  ...
  if (page.inMemoryFallback) {
    let rows = page.rows;
    if (group && group !== 'all') {
      rows = rows.filter((r) => matchesFilterGroup(mapOne(r).categories, group));
    }
    let places = rows.map(mapOne);
...
router.get('/', searchLimiter, [
  ...
  const payload = await wrap(key, async () => {
    const { places, total, qNorm } = await filterAndSort(req, { limit, offset });
