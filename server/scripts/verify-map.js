...
async function main() {
  ...
  const rows = await db.prepare('SELECT id, name, city, country, category, categories, lat, lng FROM places').all();
  ...
  const allMarkers = await listMarkers({}, 'tr');
  const museumMarkers = await listMarkers({ category: 'museum' }, 'tr');
  const natureMarkers = await listMarkers({ category: 'nature' }, 'tr');
  const historical = await listMarkers({ group: 'historical' }, 'tr');
  ...
  const { places: museumPlaces } = filterPlaces(await db.prepare('SELECT * FROM places').all(), { category: 'museum' }, statsMap);
