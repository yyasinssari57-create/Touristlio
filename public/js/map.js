window.TL_MAP = (function () {
  ...
  function popupHtml(m, lang) {
    const img = m.imageUrl
      ? (window.TL_IMG?.tag
        ? window.TL_IMG.tag(m.imageUrl, { alt: m.name, kind: 'thumb', extra: 'style="width:100%;height:72px;object-fit:cover;border-radius:8px;margin-bottom:6px"' })
        : `<img src="${m.imageUrl}" alt="${m.name || ''}" style="width:100%;height:72px;object-fit:cover;border-radius:8px;margin-bottom:6px" loading="lazy"/>`)
      : '';
    return `<div class="map-popup">
      ${img}
      <strong>${m.name}</strong>
      <div style="font-size:.72rem;color:#64748b;margin:4px 0">${m.shortDesc || ''}</div>
      <button type="button" data-act="openDetail" data-arg="${m.id}" style="font-size:.7rem;color:#0ea5e9;border:none;background:none;cursor:pointer;padding:0">
  ...
  function whenContainerReady(el, fn) {
    ...
    if (hasSize(el)) {
      try { fn(); } catch (err) { captureMapError(err); }
      return;
    }
    let done = false;
    const finish = () => {
